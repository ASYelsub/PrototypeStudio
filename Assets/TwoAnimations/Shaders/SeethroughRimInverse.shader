...
Shader "Custom/Seethrough Rim Inverse" {
    Properties {
    ...
    SubShader 
    {
        ...
        Pass
        {
            ...
            float4 frag (Interpolators i) : SV_Target
            {
                ...
                float4 value = float4(fresnelColor.rgb, saturate(fresnel.r));


                 float dSteps = max(2,_diffuseSteps);
                float falloff = value.a;
                falloff = floor(falloff * dSteps)/dSteps;

                value = float4(value.rgb, falloff);
                return value;
            }
            ENDCG
        } 
